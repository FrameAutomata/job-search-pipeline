...
**`screened-dead` is permanent** — no run ever re-checks a URL carrying it — so only a verdict that a posting is *gone* may produce it. `classify_liveness` ([pipeline/screen.py](pipeline/screen.py)) therefore resolves everything that merely means *we couldn't read it* to `throttled`/`uncertain` instead, and the tell is that most of those arrive looking like an empty page: a 403/429/999 wall (decided on status, before the body is touched), a **5xx** (the site is broken, not the posting removed), and a **200-served anti-bot challenge** (Cloudflare "Just a moment…", hCaptcha — the status check can't see these, and the challenge body is short and apply-less, so it used to reach the `insufficient content` branch and be filed dead forever). The bot-challenge patterns are matched against the whole page, JD prose included, so they run *after* the apply-control check and are spelled to need Cloudflare's own wording — a false positive holds the row on every run, meaning the job is never evaluated at all. Upstream's `liveness-core.mjs` grew the same guard for the same reason; `screen.py` is a port of it, and a port drifts.
...
- **Exit 0 no longer means "everything merged".** merge-tracker declines a row it can't read confidently (see the score cell below, or a report number marked `failed` in `batch-state.tsv`) by printing `Skipping …` — and then still archives the TSV into `merged/` and exits 0. Nothing retries it. `run_merge_tracker` scans stdout for those lines and warns, since the return code can't be trusted to carry it.
...
- **`templates/states.yml` is the status vocabulary**, and it grows. `Hired` is the 9th state; an unknown status is not inert, because the board falls back to `Evaluated`. `CANONICAL_STATES` and `app.js`'s `STATES` are both mirrors of it, and `tests/test_app_data.py` guards them against each other.
