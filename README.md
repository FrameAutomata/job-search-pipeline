...
- **CLI** — hands you a ready-to-run command for your agent (`BATCH_CLI`, default `claude`). Interactive, can pull the live JD, search the web, and drive a browser. The result panel shows the command with **Copy command** and (on Windows) **▶ Run in terminal**, which opens a new console window with the command pre-loaded. The agent always runs in *your* terminal, where its tools and cost are visible — we never run it inside the UI process. Every skill supports this path.
